...
def irreps2matrix(qua,device='cuda:0'):
    ...
    result = basis@qua * torch.tensor(5.,dtype=torch.float32)
...
def pmepol(box, axis_types, axis_indices, rcut, coord, pairs, q_local, pol, tholes, mscales, pscales, dscales):
    pme_es_pol = ADMPPmeForce(box, axis_types, axis_indices, rcut, 5e-4, 2, lpol=False, lpme=False, steps_pol=5)
...
def pme(box, coord, pairs, q_local, U_ind, pol, tholes, mscales, pscales, dscales, construct_local_frame):
    e = energy_pme(coord, box, pairs, q_local, None, None, tholes, mscales, pscales, dscales, None, 0, None, None, None, 2, False, None, None, None, False, lpme=False)
    return e
...
def convert_e3nn(q,dip,qua,device_0):
...
if __name__ == '__main__':
    ...
    model_q = load_model(configs.config_monopole(), './e3nnmodel/q_mtw.pt', device)
    model_dip = load_model(configs.config_dipole(), './e3nnmodel/dipole_mtw.pt', device)
    model_qua = load_model(configs.config_quadrupole(), './e3nnmodel/qua_mtw.pt', device)
    #model_pol = load_model(configs.config_monopole(), './model/pol.pt', device)
    #model_c6 = load_model(configs.config_monopole(), './model/c6.pt', device)
    #model_c8 = load_model(configs.config_monopole(), './model/c8.pt', device)
    #model_c10 = load_model(configs.config_monopole(), './model/c10.pt', device)
    
    ###############################################################################################################
    # then predict the tensor, calc the lr, obtain the sr energy, lr include E_es, E_pol and E_disp 
    keys = ['es', 'conf_0', 'conf_1', 'conf_2', 'dis', 'es_l', 'pol_l', 'disp_l']; npts = len(E_es_sapt)
    #scan_res = {}; 
    scan_res_lr = {}
    ...
    #for ipt in range(npts):
        ...
        if min_dis > 0.:     
            ...
            q_global_0 = convert_e3nn(q_0, dip_0, qua_0, device)
            q_global_1 = convert_e3nn(q_1, dip_1, qua_1, device)
            ...
            #e_AB_nonpol = energy_pme()
            #e_A_nonpol = energy_pme()
            #e_B_nonpol = energy_pme()
            e_AB_nonpol = pme(box, coord, pairs, Q_local, None, None, _tholes, mScales, pScales, dScales, None)
            e_A_nonpol = pme(box, coord_0, pairs_0, Q_local_0, None, None, _tholes_0, mScales, pScales, dScales, None)
            e_B_nonpol = pme(box, coord_1, pairs_1, Q_local_1, None, None, _tholes_1, mScales, pScales, dScales, None)
...
np.savez_compressed('hir_nn_sapt.npz', **scan_res_lr)
